...
Sits between `discover` and `design`: takes every scenario the product
...
rule. Like `preferences`, the output is normative but records only the
...
**Consumers:** `design` reads `docs/design/logic/` to pre-fill models
